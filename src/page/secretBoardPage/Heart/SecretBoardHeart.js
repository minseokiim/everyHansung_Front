...
const SecretBoardHeart = () => {
  ...
  useEffect(() => {
    apiClient
      .get(`${config.API_BASE_URL}/secret/heart/${studentId}/${id}`)
      .then((response) => {
  ...
  const clickHeart = () => {
    if (isFilled) {
      apiClient
        .delete(`${config.API_BASE_URL}/secret/heart/${studentId}/${id}`, {
          params: {
            studentId: studentId,
            secretboardId: id,
          },
      ...
      apiClient
        .post(`${config.API_BASE_URL}/secret/heart/${studentId}/${id}`, null, {
          params: {
            studentId: studentId,
            secretboardId: id,
          },
